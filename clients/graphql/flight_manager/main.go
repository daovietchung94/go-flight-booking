...
import (
	...
	"go-training/clients/graphql/flight_manager/resolver"
	"go-training/config"
...
func main() {
	...
	log.Infof("connect to http://%s:%s/ for GraphQL playground", conf.GraphQLConf.FlightManagerConf.Host, conf.GraphQLConf.FlightManagerConf.Port)
	log.Fatal(http.ListenAndServe(":"+conf.GraphQLConf.FlightManagerConf.Port, nil))
}
